  ...
  <div class="page-wrap">
    ...
    <main class="page-main">
      <div class="content-card">
        <div class="toolbar">
          <el-input v-model="keyword" placeholder="搜索计划名称" style="width:220px" clearable :prefix-icon="Search" @keyup.enter="() => loadList(1)" />
        ...
        <el-table :data="list" v-loading="loading" border style="width:100%" @row-click="onRowClick">
          ...
          <el-table-column label="操作" width="220" fixed="right">
            <template #default="{ row }">
              <el-button text type="success" size="small" :loading="runningId === row.id" @click.stop="showEnvDialog(row.id)">执行</el-button>
    ...
    <el-dialog v-model="envDialogVisible" title="选择运行环境" width="440px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="!!runningId" :disabled="!selectedEnvId" @click="doRun(pendingRunId)">确认执行</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { directoryApi, uiPlanApi, uiEnvApi } from '../../api';
...
async function loadDirs() {
  if (!projectId()) return;
  dirs.value = (await directoryApi.tree(projectId(), 'UI_PLAN')).data;
}
...
async function confirmAdd() {
  ...
  await directoryApi.create(name, addParentId.value, projectId(), 'UI_PLAN');
  loadDirs();
}
...
async function loadList(page = 1) {
  ...
  try {
    const res = await uiPlanApi.list({
      projectId: projectId(),
...
function goCreate() {
  router.push({ path: '/ui-auto/plan/create', query: { directoryId: selectedDir.value || undefined } as any });
}

function goEdit(row: any) {
  router.push('/ui-auto/plan/' + row.id + '/edit');
}
...
async function doRun(id: string) {
  runningId.value = id;
  try {
    const res = await uiPlanApi.run(id, selectedEnvId.value || undefined);
