  ...
  <div class="page-wrap">
    ...
    <main class="page-main">
      <div class="content-card">
        <div class="toolbar">
          <el-input v-model="keyword" placeholder="搜索场景名称" style="width:220px" clearable :prefix-icon="Search" @keyup.enter="() => loadList(1)" />
          <el-button @click="() => loadList(1)">搜索</el-button>
          <div style="flex:1" />
          <el-button type="primary" :icon="Plus" @click="openCreate">新建场景</el-button>
        </div>
        <el-table :data="list" v-loading="loading" border style="width:100%">
          <el-table-column prop="name" label="名称" min-width="200" show-overflow-tooltip fixed="left" />
          ...
          <el-table-column label="操作" width="240" fixed="right">
            <template #default="{ row }">
              <el-button text type="primary" size="small" @click.stop="$router.push('/ui-auto/scenario/' + row.id)">编辑</el-button>
    ...
    <el-dialog v-model="envDialogVisible" title="选择运行环境" width="440px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="!!runningId" :disabled="!selectedEnvId" @click="runScenario(pendingRunId)">确认运行</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="showCreate" title="新建场景" width="520px" destroy-on-close @closed="resetCreateForm">
      <el-form :model="createForm" label-width="100px">
        ...
        <el-form-item label="失败策略">
          <el-select v-model="createForm.failStrategy" style="width:100%">
...
import { directoryApi, uiScenarioApi, uiExecApi, uiEnvApi } from '../../api';
...
async function loadDirs() {
  if (!projectId()) return;
  dirs.value = (await directoryApi.tree(projectId(), 'UI_SCENARIO')).data;
}
...
async function confirmAdd() {
  ...
  await directoryApi.create(name, addParentId.value, projectId(), 'UI_SCENARIO');
  loadDirs();
}
...
const createForm = ref({ name: '', description: '', failStrategy: 'STOP' });
...
function resetCreateForm() {
  createForm.value = { name: '', description: '', failStrategy: 'STOP' };
}

async function doCreate() {
  if (!createForm.value.name.trim()) { ElMessage.warning('请输入场景名称'); return; }
  creating.value = true;
  try {
    const res = await uiScenarioApi.create({
      ...createForm.value,
      projectId: projectId(),
    ...
    router.push('/ui-auto/scenario/' + res.data.id);
...
async function loadList(page = 1) {
  ...
  try {
    const res = await uiScenarioApi.list({
      projectId: projectId(),
      directoryId: selectedDir.value ?? undefined,
...
async function runScenario(id: string) {
  ...
  runningId.value = id;
  try {
    const res = await uiExecApi.runScenario(id, pid, selectedEnvId.value || undefined);
