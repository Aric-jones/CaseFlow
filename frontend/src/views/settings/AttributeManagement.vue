<template>
  <div style="padding: 24px">
    <div style="display: flex; justify-content: space-between; margin-bottom: 16px">
      <h3>用例属性管理</h3>
      <a-button type="primary" @click="editId = null; form.name = ''; form.options = ''; form.multiSelect = false; form.nodeTypeLimit = ''; form.displayType = 'DROPDOWN'; showModal = true">
        <PlusOutlined /> 新增属性
      </a-button>
    </div>
...
const form = reactive({ name: '', options: '', multiSelect: false, nodeTypeLimit: '', displayType: 'DROPDOWN' });
...
function startEdit(attr: CustomAttribute) {
  ...
  form.multiSelect = attr.multiSelect === 1; form.nodeTypeLimit = attr.nodeTypeLimit || '';
...
async function save() {
  const data = { projectId: store.currentProject?.id, name: form.name,
    ...
    multiSelect: form.multiSelect ? 1 : 0, nodeTypeLimit: form.nodeTypeLimit || null, displayType: form.displayType };
